        ...
        {{ Form::model($ticket, ['route' => ['admin.tickets.update', $ticket->id], 'id' => 'ticket-info', 'class' => 'collapse mt-3', 'method' => 'PUT', 'enctype' => 'multipart/form-data']) }}
            <div class="row">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    <label class="require form-label">{{ __('Тіркеу:') }}
                                        <small>({{ __('документ немесе сурет') }})</small> </label>
                                            ...
                                            @if (!empty($attachments))
                                                @foreach ($attachments as $index => $attachment)
                                                    <li class="list-group-item px-0 me-3 b-0">
                                                        <a download="" href="{{ $logo.'tickets/' . $ticket->ticket_id . '/' . $attachment }}" class="btn btn-sm btn-primary d-inline-flex align-items-center" data-bs-toggle="tooltip" title="{{ __('Жүктеу') }}">
                                                            <i class="ti ti-arrow-bar-to-down me-2"></i> {{ $attachment }}
                                                        </a>
                                                        <a class="bg-danger ms-2 mx-3 btn btn-sm d-inline-flex align-items-center" title="{{ __('Жою') }}" onclick="(confirm('Сенімдісіз бе?')?(document.getElementById('user-form-{{ $index }}').submit()):'');">
                ...
                {{-- <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="card-header">
                            <h6>{{ __('Attachments') }}<small class="d-block">({{__('')}})</small></h6>
                        </div>
                        <div class="card-body">
                            <div class="choose-file form-group">
                                <label for="file" class="form-label d-block">
                                ...
                                @if (!empty($attachments))
                                    @foreach ($attachments as $index => $attachment)
                                        ...
                                        {{$attachment}}
                                            ...
                                            <a class="text-danger ml-1" title="{{ __('Delete') }}" onclick="(confirm('Сенімдісіз бе?')?(document.getElementById('user-form-{{$index}}').submit()):'');"><i class="nav-icon i-Close-Window font-weight-bold"></i></a>
                                        </li>
                                    @endforeach
                                @endif
                            </ul>
                            ...
                            <form method="post" action="{{ route('admin.conversion.store', $ticket->id) }}"
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group file-group mb-5">
                                        <label class="require form-label">{{ __('Тіркеу') }}</label>
                                        <label class="form-label"><small>({{ __('Құжат немесе сурет') }})</small></label>
                                        <div class="choose-file form-group">
                                            <label for="file" class="form-label d-block">
                                                <div>{{ __('Файлды таңдау') }}</div>
