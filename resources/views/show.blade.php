...
@push('scripts')
    ...
    <script>
        CKEDITOR.replace('reply_description', {
            ...
            editorplaceholder: '{{__('Текст енгізу...')}}'
        });
    </script>
@endpush
            ...
            <div class="col-xl-12 text-start">
                <div class="mx-3 mx-md-5">
                    <div class="card-header">
                        <h5 class="text-white">{{ __('Тикет') }} - {{$ticket->ticket_id}}</h5>
                    </div>
                </div>
                ...
                <div class="card p-4">
                                ...
                                @if(count($attachments))
                                    <div class="m-1">
                                        ...
                                        <ul class="list-group list-group-flush">
                                            ...
                                            @foreach($attachments as $index => $attachment)
                                                <li class="list-group-item px-0">
                                                    {{$attachment}}<a download="mr-2" href="{{ asset(Storage::url('tickets/'.$ticket->ticket_id."/".$attachment))  }}" class="edit-icon py-1 ml-2" title="{{ __('Жүктеп алу') }}"><i class="fa fa-download ms-2"></i></a>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                                ...
                                <form method="post" action="{{route('home.reply',$ticket->ticket_id)}}" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-12">
                                            <label class="require form-label">{{ __('Сипаттама') }}</label>
                                        ...
                                        <div class="form-group col-md-12 file-group">
                                            <label class="require form-label">{{ __('!!!') }}</label>
                                            <label class="form-label"><small>({{__('Документ немес сурет')}})</small></label>
                                    ...
                                    <div class="form-group col-md-12">
                                        <div class="text-center">
                                            ...
                                            <button class="btn btn-submit btn-primary btn-block mt-2">{{ __('Жіберу') }}</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                    ...
                    @else
                        <div class="card">
                            <div class="card-body">
                                <p class="text-blue font-weight-bold text-center mb-0">{{ __('Тикет жабық.') }}</p>
                            </div>
                        </div>
                    @endif
