...
<head>
    ...
    <title>
        @yield('page-title'){{ config('HELPDESK AUES') }}
    </title>
...
<body class="{{ $color }}">
    ...
    <script>
        var dataTableLang = {
            ...
            lengthMenu: "{{ __('Көрсету') }} _MENU_ {{ __('entries') }}",
            zeroRecords: "{{ __('Базада ақпарат жоқ.') }}",
            info: "{{ __('Көру') }} _START_ {{ __('to') }} _END_ {{ __('of') }} _TOTAL_ {{ __('entries') }}",
            infoEmpty: "{{ __('Showing 0 to 0 of 0 entries') }}",
            infoFiltered: "{{ __('(filtered from _MAX_ total entries)') }}",
            search: "{{ __('Іздеу:') }}",
            thousands: ",",
            loadingRecords: "{{ __('Жүктеу...') }}",
            processing: "{{ __('Жүктеу...') }}"
        }
    </script>
